...
def new_task():
    ...
    # return task form to create a new task
    return render_template("tasks/new.html", all_users=users)

@tasks_blueprint.route('/tasks', methods=['POST'])
def create_task():
    # grab bits from new tasks form and assign to variables
    description = request.form["description"]
    user_id = request.form["user"]
    duration = request.form["duration"]
    completed = request.form["completed"]
    # find the right user in database, from user id given from form
    user = user_repository.select(user_id)
    #create a new task object from the form data
    task = Task(description, user, duration, completed)
    # save it to the database
    task_repository.save(task)
    # redirect back to all tasks
    return redirect('/tasks')

@tasks_blueprint.route('/tasks/<id>')
def show_task(id):
    # capture id from the url
    # find th right task in the db from the id
